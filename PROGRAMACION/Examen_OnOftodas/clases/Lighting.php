...
class Lighting extends Connection {
    ...
    // Enciende o apaga todas las lámparas
public function toggleAllLamps($status) {
    $status = $status ? 1 : 0; // Asegurar valor booleano convertido a 1/0
    $sql = "UPDATE lamps SET lamp_on = :status";
    
    // Si hay filtro activo, aplicarlo
    if ($this->currentFilter !== 'all') {
        $sql .= " WHERE lamp_zone IN (SELECT zone_id FROM zones WHERE zone_name = :zone)";
    }
    ...
    if ($this->currentFilter !== 'all') {
        $stmt->bindParam(':zone', $this->currentFilter, PDO::PARAM_STR);
    }
    
    return $stmt->execute();
}

// Método para obtener el estado actual (si todas están encendidas/apagadas)
public function getGlobalStatus() {
    $sql = "SELECT 
            COUNT(*) as total,
            SUM(lamp_on) as on_count
            FROM lamps";
    ...
    if ($result['total'] == 0) return null; // No hay lámparas
    return $result['on_count'] == $result['total']; // true si todas encendidas
}
