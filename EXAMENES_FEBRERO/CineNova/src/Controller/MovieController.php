...
final class MovieController extends AbstractController
{
    ...
    public function index(MovieRepository $movieRepository, BookingRepository $bookingRepository): Response
    {
        $movies = $movieRepository->findAll();
        $movieCounters = [];
        foreach ($movies as $movie) {
            // Total reservas de una pelicula
            $total = $bookingRepository->movieCounterBooking($movie);
            // Reservas activas de una pelicula
            $active = $bookingRepository->movieCounterActiveBooking($movie);
        }
         $movieCounters[$movie->getId()] = [
             'total' => $total,
             'active' => $active
         ];
